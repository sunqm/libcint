/*
 * Copyright (C) 2021  Qiming Sun <osirpt.sun@gmail.com>
 *
 */

#include <string.h>
#include <math.h>
#include <assert.h>
#include "cint_bas.h"
#include "misc.h"
#include "g1e.h"
#include "g1e_grids.h"
#include "rys_roots.h"


void CINTinit_int1e_grids_EnvVars(CINTEnvVars *envs, FINT *ng, FINT *shls,
                                  FINT *atm, FINT natm,
                                  FINT *bas, FINT nbas, double *env)
{
        CINTinit_int1e_EnvVars(envs, ng, shls, atm, natm, bas, nbas, env);
        FINT ngrids = (FINT)env[NGRIDS];
        double *grids = env + (size_t)env[PTR_GRIDS];

        envs->ngrids = ngrids;
        envs->grids = grids;
        envs->common_factor = 2 * M_PI
                * CINTcommon_fac_sp(envs->i_l) * CINTcommon_fac_sp(envs->j_l);

        FINT nroots = envs->nrys_roots;
        FINT dli, dlj;
        FINT ibase = 1;//envs->li_ceil > envs->lj_ceil;
        if (ibase) {
                dli = envs->li_ceil + envs->lj_ceil + 1;
                dlj = envs->lj_ceil + 1;
        } else {
                dli = envs->li_ceil + 1;
                dlj = envs->li_ceil + envs->lj_ceil + 1;
        }
        envs->g_stride_i = GRID_BLKSIZE * nroots;
        envs->g_stride_j = GRID_BLKSIZE * nroots * dli;
        envs->g_size     = GRID_BLKSIZE * nroots * dli * dlj;
        envs->f_g0_2e = &CINTg0_1e_grids;

        if (ibase) {
                envs->g2d_ijmax = envs->g_stride_i;
                envs->rx_in_rijrx = envs->ri;
                envs->rirj[0] = envs->ri[0] - envs->rj[0];
                envs->rirj[1] = envs->ri[1] - envs->rj[1];
                envs->rirj[2] = envs->ri[2] - envs->rj[2];
                envs->f_g0_2d4d = &CINTg0_1e_grids_igtj;
        } else {
                envs->g2d_ijmax = envs->g_stride_j;
                envs->rx_in_rijrx = envs->rj;
                envs->rirj[0] = envs->rj[0] - envs->ri[0];
                envs->rirj[1] = envs->rj[1] - envs->ri[1];
                envs->rirj[2] = envs->rj[2] - envs->ri[2];
                envs->f_g0_2d4d = &CINTg0_1e_grids_iltj;
        }
}

#define RGSQUARE(r, ig)     (r[ig+GRID_BLKSIZE*0]*r[ig+GRID_BLKSIZE*0] + \
                             r[ig+GRID_BLKSIZE*1]*r[ig+GRID_BLKSIZE*1] + \
                             r[ig+GRID_BLKSIZE*2]*r[ig+GRID_BLKSIZE*2])

int CINTg0_1e_grids(double *g, const double fac, CINTEnvVars *envs,
                    double *cache)
{
        FINT ngrids = envs->ngrids;
        FINT bgrids = MIN(ngrids - envs->grids_offset, GRID_BLKSIZE);
        FINT nroots = envs->nrys_roots;
        double *grids = envs->grids + envs->grids_offset * 3;
        double *gx = g;
        double *gy = g + envs->g_size;
        double *gz = g + envs->g_size * 2;
        double *w = gz;
        double *rij = envs->rij;
        double ubuf[MXRYSROOTS];
        double wbuf[MXRYSROOTS];
        double *u;
        MALLOC_ALIGN8_INSTACK(u, GRID_BLKSIZE*nroots);
        double *rijrg;
        MALLOC_ALIGN8_INSTACK(rijrg, GRID_BLKSIZE*3);
        double aij = envs->aij;
        FINT n, i, j, ig;
        double x, fac1;

        for (i = 0; i < nroots; i++) {
                for (ig = 0; ig < bgrids; ig++) {
                        gx[ig+GRID_BLKSIZE*i] = 1;
                        gy[ig+GRID_BLKSIZE*i] = 1;
                }
        }
        for (ig = 0; ig < bgrids; ig++) {
                rijrg[ig+GRID_BLKSIZE*0] = grids[ig*3+0] - rij[0];
        }
        for (ig = 0; ig < bgrids; ig++) {
                rijrg[ig+GRID_BLKSIZE*1] = grids[ig*3+1] - rij[1];
        }
        for (ig = 0; ig < bgrids; ig++) {
                rijrg[ig+GRID_BLKSIZE*2] = grids[ig*3+2] - rij[2];
        }

#ifdef WITH_RANGE_COULOMB
        const double omega = envs->env[PTR_RANGE_OMEGA];
        double theta, sqrt_theta;

        if (omega == 0) {
                fac1 = fac / aij;
                for (ig = 0; ig < bgrids; ig++) {
                        x = aij * RGSQUARE(rijrg, ig);
                        CINTrys_roots(nroots, x, ubuf, wbuf);
                        for (i = 0; i < nroots; i++) {
                                u[ig+GRID_BLKSIZE*i] = ubuf[i] / (ubuf[i] + 1);
                                w[ig+GRID_BLKSIZE*i] = wbuf[i] * fac1;
                        }
                }
        } else if (omega < 0) { // short-range part of range-separated Coulomb
                theta = omega * omega / (omega * omega + aij);
                sqrt_theta = sqrt(theta);
                fac1 = fac * sqrt_theta / aij;
                for (ig = 0; ig < bgrids; ig++) {
                        x = aij * RGSQUARE(rijrg, ig);
                        if (theta * x > envs->expcutoff) {
                                // very small erfc() leads to ~0 weights
                                for (i = 0; i < nroots; i++) {
                                        u[ig+GRID_BLKSIZE*i] = 0;
                                        w[ig+GRID_BLKSIZE*i] = 0;
                                }
                        } else {
                                CINTsr_rys_roots(nroots, x, sqrt_theta, ubuf, wbuf);
                                for (i = 0; i < nroots; i++) {
                                        u[ig+GRID_BLKSIZE*i] = ubuf[i] / (ubuf[i] + 1);
                                        w[ig+GRID_BLKSIZE*i] = wbuf[i] * fac1;
                                }
                        }
                }
        } else {  // long-range part of range-separated Coulomb
                theta = omega * omega / (omega * omega + aij);
                aij *= theta;
                fac1 = fac * sqrt(theta) / aij;
                for (ig = 0; ig < bgrids; ig++) {
                        x = aij * RGSQUARE(rijrg, ig);
                        CINTrys_roots(nroots, x, ubuf, wbuf);
                        for (i = 0; i < nroots; i++) {
                                u[ig+GRID_BLKSIZE*i] = ubuf[i] / (ubuf[i] + 1 - ubuf[i] * theta);
                                w[ig+GRID_BLKSIZE*i] = wbuf[i] * fac1;
                        }
                }
        }
#else
        fac1 = fac / aij;
        for (ig = 0; ig < bgrids; ig++) {
                x = aij * RGSQUARE(rijrg, ig);
                CINTrys_roots(nroots, x, ubuf, wbuf);
                for (i = 0; i < nroots; i++) {
                        u[ig+GRID_BLKSIZE*i] = ubuf[i] / (ubuf[i] + 1);
                        w[ig+GRID_BLKSIZE*i] = wbuf[i] * fac1;
                }
        }
#endif
        FINT nmax = envs->li_ceil + envs->lj_ceil;
        if (nmax == 0) {
                return 1;
        }

        FINT lj = envs->lj_ceil;
        FINT di = envs->g2d_ijmax;
        FINT dj = envs->g_stride_j;

        double *rijrx = envs->rijrx;
        double *p0x, *p0y, *p0z;
        double *p1x, *p1y, *p1z;
        double *p2x, *p2y, *p2z;
        double *rirg;
        MALLOC_ALIGN8_INSTACK(rirg, GRID_BLKSIZE*3);
        double *t2;
        MALLOC_ALIGN8_INSTACK(t2, GRID_BLKSIZE*nroots);
        double tmp;
        double aij2 = 0.5 / aij;
        for (n = 0; n < nroots; n++) {
                for (ig = 0; ig < bgrids; ig++) {
                        t2[ig+GRID_BLKSIZE*n] = aij2 * (1 - u[ig+GRID_BLKSIZE*n]);
                }
        }

        for (n = 0; n < nroots; n++) {
                p0x = gx + GRID_BLKSIZE*n;
                p1x = p0x + di;
                for (ig = 0; ig < bgrids; ig++) {
                        rirg[ig+GRID_BLKSIZE*0] = rijrx[0] + u[ig+GRID_BLKSIZE*n] * rijrg[ig+GRID_BLKSIZE*0];
                }

                for (ig = 0; ig < bgrids; ig++) {
                        p1x[ig] = rirg[ig+GRID_BLKSIZE*0] * p0x[ig];
                }

                for (i = 1; i < nmax; i++) {
                        p0x = gx + GRID_BLKSIZE*n + i * di;
                        p1x = p0x + di;
                        p2x = p0x - di;
                        for (ig = 0; ig < bgrids; ig++) {
                                p1x[ig] = i * t2[ig+GRID_BLKSIZE*n] * p2x[ig] + rirg[ig+GRID_BLKSIZE*0] * p0x[ig];
                        }
                }
        }

        for (n = 0; n < nroots; n++) {
                p0y = gy + GRID_BLKSIZE*n;
                p1y = p0y + di;
                for (ig = 0; ig < bgrids; ig++) {
                        rirg[ig+GRID_BLKSIZE*1] = rijrx[1] + u[ig+GRID_BLKSIZE*n] * rijrg[ig+GRID_BLKSIZE*1];
                }

                for (ig = 0; ig < bgrids; ig++) {
                        p1y[ig] = rirg[ig+GRID_BLKSIZE*1] * p0y[ig];
                }

                for (i = 1; i < nmax; i++) {
                        p0y = gy + GRID_BLKSIZE*n + i * di;
                        p1y = p0y + di;
                        p2y = p0y - di;
                        for (ig = 0; ig < bgrids; ig++) {
                                p1y[ig] = i * t2[ig+GRID_BLKSIZE*n] * p2y[ig] + rirg[ig+GRID_BLKSIZE*1] * p0y[ig];
                        }
                }
        }

        for (n = 0; n < nroots; n++) {
                p0z = gz + GRID_BLKSIZE*n;
                p1z = p0z + di;
                for (ig = 0; ig < bgrids; ig++) {
                        rirg[ig+GRID_BLKSIZE*2] = rijrx[2] + u[ig+GRID_BLKSIZE*n] * rijrg[ig+GRID_BLKSIZE*2];
                }

                for (ig = 0; ig < bgrids; ig++) {
                        p1z[ig] = rirg[ig+GRID_BLKSIZE*2] * p0z[ig];
                }

                for (i = 1; i < nmax; i++) {
                        p0z = gz + GRID_BLKSIZE*n + i * di;
                        p1z = p0z + di;
                        p2z = p0z - di;
                        for (ig = 0; ig < bgrids; ig++) {
                                p1z[ig] = i * t2[ig+GRID_BLKSIZE*n] * p2z[ig] + rirg[ig+GRID_BLKSIZE*2] * p0z[ig];
                        }
                }
        }

        (*envs->f_g0_2d4d)(g, envs);
        return 1;
}

void CINTg0_1e_grids_igtj(double *g, CINTEnvVars *envs)
{
        FINT ngrids = envs->ngrids;
        FINT bgrids = MIN(ngrids - envs->grids_offset, GRID_BLKSIZE);
        FINT nroots = envs->nrys_roots;
        FINT nmax = envs->li_ceil + envs->lj_ceil;
        FINT lj = envs->lj_ceil;
        FINT di = envs->g_stride_i;
        FINT dj = envs->g_stride_j;
        double *gx = g;
        double *gy = g + envs->g_size;
        double *gz = g + envs->g_size * 2;
        double *p0x, *p0y, *p0z;
        double *p1x, *p1y, *p1z;
        double *p2x, *p2y, *p2z;
        double *rirj = envs->rirj;
        FINT i, j, ig, n;

        for (j = 1; j <= lj; j++) {
        for (i = 0; i <= nmax - j; i++) {
                p0x = gx + j * dj + i * di;
                p1x = p0x - dj;
                p2x = p1x + di;
                for (n = 0; n < nroots; n++) {
                for (ig = 0; ig < bgrids; ig++) {
                        p0x[ig+GRID_BLKSIZE*n] = p2x[ig+GRID_BLKSIZE*n] + rirj[0] * p1x[ig+GRID_BLKSIZE*n];
                } }
        } }

        for (j = 1; j <= lj; j++) {
        for (i = 0; i <= nmax - j; i++) {
                p0y = gy + j * dj + i * di;
                p1y = p0y - dj;
                p2y = p1y + di;
                for (n = 0; n < nroots; n++) {
                for (ig = 0; ig < bgrids; ig++) {
                        p0y[ig+GRID_BLKSIZE*n] = p2y[ig+GRID_BLKSIZE*n] + rirj[1] * p1y[ig+GRID_BLKSIZE*n];
                } }
        } }

        for (j = 1; j <= lj; j++) {
        for (i = 0; i <= nmax - j; i++) {
                p0z = gz + j * dj + i * di;
                p1z = p0z - dj;
                p2z = p1z + di;
                for (n = 0; n < nroots; n++) {
                for (ig = 0; ig < bgrids; ig++) {
                        p0z[ig+GRID_BLKSIZE*n] = p2z[ig+GRID_BLKSIZE*n] + rirj[2] * p1z[ig+GRID_BLKSIZE*n];
                } }
        } }
}

void CINTg0_1e_grids_iltj(double *g, CINTEnvVars *envs)
{
        FINT ngrids = envs->ngrids;
        FINT bgrids = MIN(ngrids - envs->grids_offset, GRID_BLKSIZE);
        FINT nroots = envs->nrys_roots;
        FINT nmax = envs->li_ceil + envs->lj_ceil;
        FINT li = envs->li_ceil;
        FINT di = envs->g_stride_i;
        FINT dj = envs->g_stride_j;
        double *gx = g;
        double *gy = g + envs->g_size;
        double *gz = g + envs->g_size * 2;
        double *p0x, *p0y, *p0z;
        double *p1x, *p1y, *p1z;
        double *p2x, *p2y, *p2z;
        double *rirj = envs->rirj;
        FINT i, j, ig, n;

        for (i = 1; i <= li; i++) {
        for (j = 0; j <= nmax-i; j++) {
                p0x = gx + j * dj + i * di;
                p1x = p0x - di;
                p2x = p1x + dj;
                for (n = 0; n < nroots; n++) {
                for (ig = 0; ig < bgrids; ig++) {
                        p0x[ig+GRID_BLKSIZE*n] = p2x[ig+GRID_BLKSIZE*n] + rirj[0] * p1x[ig+GRID_BLKSIZE*n];
                } }
        } }

        for (i = 1; i <= li; i++) {
        for (j = 0; j <= nmax-i; j++) {
                p0y = gy + j * dj + i * di;
                p1y = p0y - di;
                p2y = p1y + dj;
                for (n = 0; n < nroots; n++) {
                for (ig = 0; ig < bgrids; ig++) {
                        p0y[ig+GRID_BLKSIZE*n] = p2y[ig+GRID_BLKSIZE*n] + rirj[1] * p1y[ig+GRID_BLKSIZE*n];
                } }
        } }

        for (i = 1; i <= li; i++) {
        for (j = 0; j <= nmax-i; j++) {
                p0z = gz + j * dj + i * di;
                p1z = p0z - di;
                p2z = p1z + dj;
                for (n = 0; n < nroots; n++) {
                for (ig = 0; ig < bgrids; ig++) {
                        p0z[ig+GRID_BLKSIZE*n] = p2z[ig+GRID_BLKSIZE*n] + rirj[2] * p1z[ig+GRID_BLKSIZE*n];
                } }
        } }
}

void CINTgout1e_grids(double *gout, double *g, FINT *idx,
                      CINTEnvVars *envs, FINT gout_empty)
{
        FINT ngrids = envs->ngrids;
        FINT bgrids = MIN(ngrids - envs->grids_offset, GRID_BLKSIZE);
        FINT nroots = envs->nrys_roots;
        FINT nf = envs->nf;
        FINT i, n, ig;
        double *gx, *gy, *gz;
        double s[GRID_BLKSIZE];

        if (gout_empty) {
                for (n = 0; n < nf; n++, idx+=3) {
                        gx = g + idx[0];
                        gy = g + idx[1];
                        gz = g + idx[2];
                        for (ig = 0; ig < bgrids; ig++) {
                                s[ig] = 0;
                        }
                        for (i = 0; i < nroots; i++) {
                                for (ig = 0; ig < bgrids; ig++) {
                                        s[ig] += gx[ig+GRID_BLKSIZE*i] * gy[ig+GRID_BLKSIZE*i] * gz[ig+GRID_BLKSIZE*i];
                                }
                        }
                        for (ig = 0; ig < bgrids; ig++) {
                                gout[ig+bgrids*n] = s[ig];
                        }
                }
        } else {
                for (n = 0; n < nf; n++, idx+=3) {
                        gx = g + idx[0];
                        gy = g + idx[1];
                        gz = g + idx[2];
                        for (ig = 0; ig < bgrids; ig++) {
                                s[ig] = 0;
                        }
                        for (i = 0; i < nroots; i++) {
                                for (ig = 0; ig < bgrids; ig++) {
                                        s[ig] += gx[ig+GRID_BLKSIZE*i] * gy[ig+GRID_BLKSIZE*i] * gz[ig+GRID_BLKSIZE*i];
                                }
                        }
                        for (ig = 0; ig < bgrids; ig++) {
                                gout[ig+bgrids*n] += s[ig];
                        }
                }
        }
}

void CINTnabla1i_grids(double *f, double *g,
                       FINT li, FINT lj, CINTEnvVars *envs)
{
        FINT ngrids = envs->ngrids;
        FINT bgrids = MIN(ngrids - envs->grids_offset, GRID_BLKSIZE);
        const FINT di = envs->g_stride_i;
        const FINT dj = envs->g_stride_j;
        const double ai2 = -2 * envs->ai;
        FINT i, j, ig, ptr;
        const double *gx = g;
        const double *gy = g + envs->g_size;
        const double *gz = g + envs->g_size * 2;
        double *fx = f;
        double *fy = f + envs->g_size;
        double *fz = f + envs->g_size * 2;

        for (j = 0; j <= lj; j++) {
                ptr = dj * j;
                //f(...,0,...) = -2*ai*g(...,1,...)
                for (ig = ptr; ig < ptr+bgrids; ig++) {
                        fx[ig] = ai2 * gx[ig+di];
                }
                //f(...,i,...) = i*g(...,i-1,...)-2*ai*g(...,i+1,...)
                for (i = 1; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fx[ig] = i * gx[ig-di] + ai2 * gx[ig+di];
                        }
                }
        }

        for (j = 0; j <= lj; j++) {
                ptr = dj * j;
                //f(...,0,...) = -2*ai*g(...,1,...)
                for (ig = ptr; ig < ptr+bgrids; ig++) {
                        fy[ig] = ai2 * gy[ig+di];
                }
                //f(...,i,...) = i*g(...,i-1,...)-2*ai*g(...,i+1,...)
                for (i = 1; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fy[ig] = i * gy[ig-di] + ai2 * gy[ig+di];
                        }
                }
        }

        for (j = 0; j <= lj; j++) {
                ptr = dj * j;
                //f(...,0,...) = -2*ai*g(...,1,...)
                for (ig = ptr; ig < ptr+bgrids; ig++) {
                        fz[ig] = ai2 * gz[ig+di];
                }
                //f(...,i,...) = i*g(...,i-1,...)-2*ai*g(...,i+1,...)
                for (i = 1; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fz[ig] = i * gz[ig-di] + ai2 * gz[ig+di];
                        }
                }
        }
}

void CINTnabla1j_grids(double *f, double *g,
                       FINT li, FINT lj, CINTEnvVars *envs)
{
        FINT ngrids = envs->ngrids;
        FINT bgrids = MIN(ngrids - envs->grids_offset, GRID_BLKSIZE);
        const FINT di = envs->g_stride_i;
        const FINT dj = envs->g_stride_j;
        const double aj2 = -2 * envs->aj;
        FINT i, j, ig, ptr;
        const double *gx = g;
        const double *gy = g + envs->g_size;
        const double *gz = g + envs->g_size * 2;
        double *fx = f;
        double *fy = f + envs->g_size;
        double *fz = f + envs->g_size * 2;

        //f(...,0,...) = -2*aj*g(...,1,...)
        for (i = 0; i <= li; i++) {
                ptr = di * i;
                for (ig = ptr; ig < ptr+bgrids; ig++) {
                        fx[ig] = aj2 * gx[ig+dj];
                }
        }
        //f(...,j,...) = j*g(...,j-1,...)-2*aj*g(...,j+1,...)
        for (j = 1; j <= lj; j++) {
                for (i = 0; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fx[ig] = j * gx[ig-dj] + aj2 * gx[ig+dj];
                        }
                }
        }

        //f(...,0,...) = -2*aj*g(...,1,...)
        for (i = 0; i <= li; i++) {
                ptr = di * i;
                for (ig = ptr; ig < ptr+bgrids; ig++) {
                        fy[ig] = aj2 * gy[ig+dj];
                }
        }
        //f(...,j,...) = j*g(...,j-1,...)-2*aj*g(...,j+1,...)
        for (j = 1; j <= lj; j++) {
                for (i = 0; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fy[ig] = j * gy[ig-dj] + aj2 * gy[ig+dj];
                        }
                }
        }

        //f(...,0,...) = -2*aj*g(...,1,...)
        for (i = 0; i <= li; i++) {
                ptr = di * i;
                for (ig = ptr; ig < ptr+bgrids; ig++) {
                        fz[ig] = aj2 * gz[ig+dj];
                }
        }
        //f(...,j,...) = j*g(...,j-1,...)-2*aj*g(...,j+1,...)
        for (j = 1; j <= lj; j++) {
                for (i = 0; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fz[ig] = j * gz[ig-dj] + aj2 * gz[ig+dj];
                        }
                }
        }
}


void CINTx1i_grids(double *f, double *g, double *ri,
                   FINT li, FINT lj, CINTEnvVars *envs)
{
        FINT ngrids = envs->ngrids;
        FINT bgrids = MIN(ngrids - envs->grids_offset, GRID_BLKSIZE);
        FINT i, j, ig, ptr;
        const FINT di = envs->g_stride_i;
        const FINT dj = envs->g_stride_j;
        const double *gx = g;
        const double *gy = g + envs->g_size;
        const double *gz = g + envs->g_size * 2;
        double *fx = f;
        double *fy = f + envs->g_size;
        double *fz = f + envs->g_size * 2;

        for (j = 0; j <= lj; j++) {
                for (i = 0; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fx[ig] = gx[ig+di] + ri[0] * gx[ig];
                        }
                }
        }

        for (j = 0; j <= lj; j++) {
                for (i = 0; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fy[ig] = gy[ig+di] + ri[1] * gy[ig];
                        }
                }
        }

        for (j = 0; j <= lj; j++) {
                for (i = 0; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fz[ig] = gz[ig+di] + ri[2] * gz[ig];
                        }
                }
        }
}

void CINTx1j_grids(double *f, double *g, double *rj,
                   FINT li, FINT lj, CINTEnvVars *envs)
{
        FINT ngrids = envs->ngrids;
        FINT bgrids = MIN(ngrids - envs->grids_offset, GRID_BLKSIZE);
        FINT i, j, ig, ptr;
        const FINT di = envs->g_stride_i;
        const FINT dj = envs->g_stride_j;
        const double *gx = g;
        const double *gy = g + envs->g_size;
        const double *gz = g + envs->g_size * 2;
        double *fx = f;
        double *fy = f + envs->g_size;
        double *fz = f + envs->g_size * 2;

        for (j = 0; j <= lj; j++) {
                for (i = 0; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fx[ig] = gx[ig+dj] + rj[0] * gx[ig];
                        }
                }
        }

        for (j = 0; j <= lj; j++) {
                for (i = 0; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fy[ig] = gy[ig+dj] + rj[1] * gy[ig];
                        }
                }
        }

        for (j = 0; j <= lj; j++) {
                for (i = 0; i <= li; i++) {
                        ptr = dj * j + di * i;
                        for (ig = ptr; ig < ptr+bgrids; ig++) {
                                fz[ig] = gz[ig+dj] + rj[2] * gz[ig];
                        }
                }
        }
}
