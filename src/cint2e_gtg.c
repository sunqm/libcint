...
int CINTinit_int2e_coul_gtg_EnvVars(CINTEnvVars *envs, int *ng, int *shls,
...
int int2e_coul_gtg_sph(double *out, int *dims, int *shls, int *atm, int natm,
                      int *bas, int nbas, double *env, CINTOpt *opt, double *cache)
{
        int ng[] = {0, 0, 0, 0, 0, 1, 1, 1};
        CINTEnvVars envs;
        CINTinit_int2e_coul_gtg_EnvVars(&envs, ng, shls, atm, natm, bas, nbas, env);
        envs.f_gout = &CINTgout2e;
        return CINT2e_spheric_drv(out, dims, &envs, opt, cache);
}
void int2e_coul_gtg_optimizer(CINTOpt **opt, int *atm, int natm,
