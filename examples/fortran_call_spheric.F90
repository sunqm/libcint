...
integer,external :: cgtos_spheric
...
!
i = 0; shls(1) = i; di = cgtos_spheric(i, bas)
j = 0; shls(2) = j; dj = cgtos_spheric(j, bas)
...
!
i = 0; shls(1) = i; di = cgtos_spheric(i, bas)
j = 1; shls(2) = j; dj = cgtos_spheric(j, bas)
k = 2; shls(3) = k; dk = cgtos_spheric(k, bas)
l = 2; shls(4) = l; dl = cgtos_spheric(l, bas)
allocate (buf2e(di,dj,dk,dl,3))
if (0 /= cint2e_ip1_sph(buf2e, shls, atm, natm, bas, nbas, env)) then
  print*, "This gradient integral is not 0.\n"
endif
deallocate (buf2e)
